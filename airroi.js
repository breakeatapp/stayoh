...
function buildMarketSummary(listings, city) {
  ...
  const prices = priced
    .map(function(item) { return item.prix; })
    .filter(function(value) { return value != null; })
    .sort(function(a, b) { return a - b; });
  const occupations = priced
...
function computeListingScore(listing, market) {
  ...
  if (listing.variation != null) {
    score += clamp(-listing.variation * 1.8, -30, 30);
  }

  if (listing.note != null) {
    score += clamp((listing.note - 4.5) * 18, -12, 12);
  }

  if (listing.avis != null) {
    score += clamp(listing.avis / 20, 0, 8);
  }

  if (listing.occupation != null) {
    score += clamp((listing.occupation - 60) / 4, -6, 10);
  }

  if (market && market.prix_moyen != null && market.bon_deal_max != null && listing.prix != null && listing.prix <= market.bon_deal_max) {
    score += 5;
  }
...
function buildInsight(listing, market, comparables) {
  ...
  if (listing.variation != null) {
    if (listing.variation < 0) {
      parts.push("Prix " + Math.abs(listing.variation) + "% sous le marché local.");
    } else if (listing.variation > 0) {
      parts.push("Prix " + listing.variation + "% au-dessus du marché local.");
    } else {
      parts.push("Prix dans la moyenne du marché local.");
    }
  }

  if (listing.note != null) {
    parts.push("Note voyageurs: " + listing.note + "/5.");
  }

  if (comparables.length) {
    parts.push(comparables.length + " comparable(s) renvoyé(s) par le backend.");
  }

  if (market && market.prix_moyen != null) {
    parts.push("Prix médian observé: " + market.prix_moyen + "€/nuit.");
  }
...
function pickListing(listings, listingId, listingUrl) {
  const normalizedId = listingId ? String(listingId) : "";
  const normalizedUrl = listingUrl ? String(listingUrl) : "";

  return listings.find(function(item) {
    return normalizedId && item.id && String(item.id) === normalizedId;
  }) || listings.find(function(item) {
    return normalizedUrl && item.url && item.url === normalizedUrl;
  }) || listings[0] || null;
}
...
function buildListingUrl(id) {
  return "https://www.airbnb.fr/rooms/" + encodeURIComponent(String(id));
}

function buildSearchUrl(city, guests) {
  return "https://www.airbnb.fr/s/" + encodeURIComponent(city) + "/homes?adults=" + encodeURIComponent(String(guests || 2));
}
...
function inferCityFromUrl(value) {
  ...
  const match = raw.match(/\/s\/([^/]+)\/homes/i);
  if (!match) return "";
